 ...
 *
 */
public class Gender extends InfoBead implements Runnable {
	...
	public void initialize() 
	{
		...
		Thread genderThread = new Thread(this,"Gender");
		genderThread.start();				
	}
	...
	public void handleData(Triplet inTriplet) 
	{
		...
		Triplet tripletInstance = new Triplet("Gender");
		
		InfoItem inputData = inTriplet.getInfoItem();
		Person myself = new Person();
		myself.IBUMDeserialize((String)inputData.getInfoValue());
		
		GenderType gnd = myself.getGender(); 		
		String gndString = (gnd == GenderType.MALE)?"MALE":"FEMALE";
		
		Time t = new Time(System.currentTimeMillis());
		InfoItem data = new InfoItem();
		data.setInferenceTime(t);
		data.setExplainInfo("Descendants >> kids");
		data.setInfoType("Humans");
		data.setInfoUnits("Kid");
		data.setInfoValue(gndString);  
